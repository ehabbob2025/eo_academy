...
export function LessonPage({ state }: { state: any }) {
  ...
  // تحميل تفاصيل الفيديو من جدول lesson_media وجلب معرف المستخدم فوراً
  useEffect(() => {
    if (!supabase || !lesson?.id) return
    const initData = async () => {
      const [{ data: auth }, { data: mediaData }] = await Promise.all([
        supabase.auth.getUser(),
        supabase.from('lesson_media').select('youtube_video_id').eq('lesson_id', lesson.id).maybeSingle()
      ])
      
      if (auth.user?.id) setCommentUserId(auth.user.id)
      if (mediaData?.youtube_video_id) setVideoId(mediaData.youtube_video_id)
  ...
  // مؤقت حساب وقت المشاهدة (يبدأ فوراً ويتحرك تلقائياً)
  useEffect(() => {
    if (isWatchedEnough) return
    ...
    const interval = window.setInterval(() => {
      if (document.visibilityState === 'visible') {
        setWatchedSeconds((prev) => {
          ...
          } else if (commentUserId && lesson?.id && next % 10 === 0) {
            void supabase.from('lesson_progress').upsert({
              user_id: commentUserId,
              lesson_id: lesson.id,
              watched_seconds: next,
    ...
    <div className="lesson-page">
      ...
      <div className="lesson-progress-box" style={{ background: '#f8fafc', border: '1px solid #e2e8f0', padding: '16px 20px', borderRadius: '12px', margin: '20px 0' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px', fontSize: '14px', fontWeight: '700', color: '#1e293b' }}>
          <span>تقدم المشاهدة ({progressPercent}%)</span>
          <span>{isWatchedEnough ? '✅ تم إكمال النسبة المطلوبة (85%)' : 'يلزم إكمال 85% لفتح المحاضرة التالية.'}</span>
        </div>
