...
export function LessonPage({ state }: { state: CourseState }) {
  ...
  // ربط كود المحاضرة الأولى 6vmVtWChcoo تلقائياً ومباشرة لضمان تشغيله فوراً
  const initialId = lesson?.position === 1 ? '6vmVtWChcoo' : (lesson?.youtubeVideoId || '')
  const [videoId, setVideoId] = useState(extractYoutubeId(initialId))
  ...
  useEffect(() => {
    ...
    const fallback = lesson?.position === 1 ? '6vmVtWChcoo' : (lesson?.youtubeVideoId || '')
    setVideoId(extractYoutubeId(fallback))
    setResult(null)
    setSelected({})
  }, [lesson?.id])
  ...
  // جلب الفيديو من Supabase مع فحص كلا الجدولين لضمان العثور على الرابط
  useEffect(() => {
    if (!supabase || !lesson) return
    const client = supabase
    const loadVideo = async () => {
      const { data: mediaData } = await client.from('lesson_media').select('youtube_video_id').eq('lesson_id', lesson.id).maybeSingle()
      if (mediaData?.youtube_video_id) {
        setVideoId(extractYoutubeId(mediaData.youtube_video_id))
        return
      }

      const { data: lessonData } = await client.from('lessons').select('youtube_video_id, youtube_url, video_url').eq('id', lesson.id).maybeSingle()
      const fallbackId = (lessonData as any)?.youtube_video_id || (lessonData as any)?.youtube_url || (lessonData as any)?.video_url
      if (fallbackId) {
        setVideoId(extractYoutubeId(fallbackId))
      }
    }
    void loadVideo()
  }, [lesson?.id])
  ...
  const embedSrc = videoId ? `https://www.youtube.com/embed/${videoId}?rel=0&modestbranding=1` : ''
  ...
  return (
    <div className="lesson-page">
      ...
      {/* مشغل الفيديو بنطاق youtube.com القياسي */}
      {videoId ? (
      ...
      <div className="lesson-navigation">
        ...
        {nextLesson && (
          <Link to={`/lesson/${nextLesson.id}`}>المحاضرة التالية <ArrowLeft size={18} /></Link>
        )}
      </div>
    </div>
  )
