...
type DbLesson = { id: string; position: number; title: string; description: string; duration_minutes: number; is_published: boolean; quiz_enabled: boolean; lesson_media: { youtube_video_id: string }[] | null }
...
export function AdminPage({ state: _state }: { state: CourseState }) {
  ...
  const addLesson = async () => {
    ...
    const { error } = await supabase.from('lessons').insert({ course_id: '11111111-1111-4111-8111-111111111111', position, title: `المحاضرة ${position}`, description: '', duration_minutes: 10, is_published: false, quiz_enabled: true })
  ...
  const loadQuiz = async (lesson: DbLesson) => {
    if (!supabase) return
    if (!lesson.quiz_enabled) { setNotice('فعّل «يوجد اختبار بعد المحاضرة» أولًا.'); return }
  ...
  const saveLesson = async (event: FormEvent<HTMLFormElement>, lessonId: string) => {
    ...
    const { error } = await supabase.from('lessons').update({ title: String(form.get('title') || '').trim(), description: String(form.get('description') || '').trim(), duration_minutes: Number(form.get('duration') || 0), is_published: form.get('published') === 'on', quiz_enabled: form.get('quizEnabled') === 'on' }).eq('id', lessonId)
  ...
  const deleteConversation = async (thread: Thread) => {
    if (!supabase || deletingThread) return
    const studentName = thread.profiles?.full_name || 'الطالب'
    if (!window.confirm(`حذف محادثة «${studentName}» بكل الرسائل نهائيًا؟`)) return
    setDeletingThread(thread.id)
    setNotice('')
    const { data: deletedThread, error } = await supabase.from('support_threads').delete().eq('id', thread.id).select('id').maybeSingle()
    setDeletingThread('')
    if (error || !deletedThread) {
      setNotice('تعذر حذف المحادثة. شغّل تحديث صلاحيات المحادثات في Supabase ثم جرّب تاني.')
      return
    }
    const remainingThreads = threads.filter((item) => item.id !== thread.id)
    setThreads(remainingThreads)
    setMessages([])
    setSelectedThread(remainingThreads[0]?.id || null)
    setNotice('تم حذف المحادثة بكل رسائلها.')
    void refreshUnreadCount(adminId)
  }
